...
function Choice<T extends string>({ value, selected, label, emoji, onClick }: {
  value: T; selected: boolean; label: string; emoji?: string; onClick: (value: T) => void
}) {
...
function OnboardingPage() {
  ...
  const toggleUserType = (value: UserType) => {
    setForm((prev) => ({ ...prev, userTypes: prev.userTypes.includes(value) ? prev.userTypes.filter((item) => item !== value) : [...prev.userTypes, value] }))
  }
  ...
  const next = async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || '가입 정보를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const setTerm = (key: keyof typeof form.terms, value: boolean) => setForm((prev) => ({ ...prev, terms: { ...prev.terms, [key]: value } }))
  const setNotification = (key: keyof typeof form.notifications) => setForm((prev) => ({ ...prev, notifications: { ...prev.notifications, [key]: !prev.notifications[key] } }))
...
function MainPage() {
  return <main className="main-page"><h1>메인화면입니다</h1></main>
}
